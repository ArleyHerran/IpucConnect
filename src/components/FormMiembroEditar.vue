<template>
  <div class="text-center pa-4">
    <v-dialog
      v-model="estados.formMiembrosEditar.display"
      ...
      <v-card>
        <v-card-title class="dialog-title" style="background: #eceff1">
          <div style="display: flex">
            <h2>EditarRegistro</h2>
            <v-spacer></v-spacer>
            <v-btn
              @click="estados.formMiembrosEditar.display = false"
          ...
          <v-form ref="form1" @submit="saveMember">
              ...
              <v-row>
                ...
                <v-col cols="12" md="3" sm="4">
                  <v-text-field
                    ...
                    label="Nombre"
                    :rules="createValidationRules(true, 4)"
                  ></v-text-field>
                </v-col>
                <v-col cols="12" md="3" sm="4">
                  <v-text-field
            ...
            <section class="sectionA">
              ...
              <v-row>
                ...
                >
                  <v-file-input
                    label="Ajuntar soporte Eclesiástico"
                    accept=".pdf,.docx"
...
const saveMember = async () => {
  ...
  dataDefault.datosRegistro.timestamp = serverTimestamp();
  ...
  if (soporteTribunal.value != null) {
    ...
    if (!uploadSuccess) {
      ...
      swal("Error", "Hubo un problema al subir el archivo. Inténtalo de nuevo.", "error");
      return; // Detiene la función si la subida del archivo falla
    }

    // Si la subida fue exitosa, se guarda la URL del archivo
    miembro.soporteTribunal = downloadURLOrError;
